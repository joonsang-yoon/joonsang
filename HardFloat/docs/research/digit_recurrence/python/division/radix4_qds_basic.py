...
QUOTIENT_CONDITIONS = {
    "positive_D": {
        "D_values": [8, 9, 10, 11, 12, 13, 14, 15],
        "q_conditions": [
            (2, lambda d, t: ((4 / 3) * (d + 1) <= t) & (t < (8 / 3) * (d + 1))),
            (1, lambda d, t: ((1 / 3) * (d + 1) <= t) & (t <= (5 / 3) * d - 2)),
            (0, lambda d, t: (-(2 / 3) * d <= t) & (t <= (2 / 3) * d - 2)),
            (-1, lambda d, t: (-(5 / 3) * d <= t) & (t <= -(1 / 3) * (d + 1) - 2)),
            (-2, lambda d, t: (-(8 / 3) * (d + 1) - 2 < t) & (t <= -(4 / 3) * (d + 1) - 2)),
        ],
    },
    "negative_D": {
        "D_values": [-16, -15, -14, -13, -12, -11, -10, -9],
        "q_conditions": [
            (2, lambda d, t: ((8 / 3) * d - 2 < t) & (t <= (4 / 3) * d - 2)),
            (1, lambda d, t: ((5 / 3) * (d + 1) <= t) & (t <= (1 / 3) * d - 2)),
            (0, lambda d, t: ((2 / 3) * (d + 1) <= t) & (t <= -(2 / 3) * (d + 1) - 2)),
            (-1, lambda d, t: (-(1 / 3) * d <= t) & (t <= -(5 / 3) * (d + 1) - 2)),
            (-2, lambda d, t: (-(4 / 3) * d <= t) & (t < -(8 / 3) * d)),
        ],
    },
}
...
def create_custom_colormap():
    colors = [
        "#e6194b",  # Red (q=2)
        "#f58231",  # Orange (q=2/q=1 overlap)
        "#ffe119",  # Yellow (q=1)
        "#bcf60c",  # Yellow-Green (q=1/q=0 overlap)
        "#3cb44b",  # Green (q=0)
        "#46f0f0",  # Cyan-Green (q=0/q=-1 overlap)
        "#008080",  # Teal (q=-1)
        "#4363d8",  # Blue-Cyan (q=-1/q=-2 overlap)
        "#000080",  # Navy (q=-2)
    ]
...
def plot_quotient_regions(D, T, q, overlap_q2_q1, overlap_q1_q0, overlap_q0_qm1, overlap_qm1_qm2, quadrants):
    ...
    plot_scatter(ax, D_masked, T_masked, q_masked == 2, color_mapping[2], "q=2", **scatter_params)
    plot_scatter(ax, D_masked, T_masked, q_masked == 1, color_mapping[1], "q=1", **scatter_params)
    plot_scatter(ax, D_masked, T_masked, q_masked == 0, color_mapping[0], "q=0", **scatter_params)
    plot_scatter(ax, D_masked, T_masked, q_masked == -1, color_mapping[-1], "q=-1", **scatter_params)
    plot_scatter(ax, D_masked, T_masked, q_masked == -2, color_mapping[-2], "q=-2", **scatter_params)
    ...
    filename = f"radix4_qds_basic_{quadrants}.png"
...
def main():
    try:
        ...
        # Plot the results for quadrants 2 and 3
        plot_quotient_regions(
            D,
            T,
            q,
