...
To initialize the recurrence, we define the first residual $R_1$ and the first quotient digit $q_1$.
...
We define the first quotient approximation as $Q_1 = Q_0 + q_1 B^{-1}$. By setting $Q_0 = 0$, the
first digit $q_1$ is chosen from the non-negative half of the digit set,
$q_1 \in \{0, 1, \dotsc, a\}$.
...
In what follows, $\Int(\cdot)$ denotes the integer value of the shown fixed-point word obtained by
removing the binary point (equivalently, scaling by $2^\alpha$ or $2^\beta$ and interpreting the
...
The analysis proceeds analogously using
...
To initialize the square root recurrence, we define the first residual $R_1$ and the first root
digit $s_1$.
The recurrence for $j \geq 1$ is:
...
approximation as $S_1 = S_0 + s_1 B^{-1}$. By setting $S_0 = 1$, the first digit $s_1$ is chosen
from the non-positive half of the digit set, $s_1 \in \{0, -1, \dotsc, -a\}$.
Recall that $\rho = \frac{a}{B - 1}$ and $\frac{1}{2} < \rho \leq 1$.
...
Here we write the binary expansion of $S_j \in [1/2,1)$ as
$S_j = 0.1u_{j,-2}u_{j,-3}\dots$ (the $u$'s are bits of $S_j$, not SRT digits). Also,
$\ulp = 2^{-j}$, so $\sigma_j \leq 2^\beta S_j \leq \sigma_j + 1 - 2^\beta 2^{-j}$.

Continuity analysis uses $\min(U_{k-1,\mathrm{trunc}}) \geq \max(L_{k,\mathrm{trunc}})$ with
$\rho = 1$,
$B = 2$:
...
This makes overlap iteration-dependent and most restrictive for small $j$ and for negative digits
with larger squared terms.
...
This failure for $j = 1, 2$ indicates the need for a special startup mechanism (or more precision)
...
In \cref{ssec:sqrt:radix4}, we observed that under the absolute worst-case assumption for the
partial root ($\sigma_j = 8$), the continuity condition fails for iterations $j = 1$ and $j = 2$.
...
However, the worst-case $\sigma_j$ does not occur in all execution paths. The actual value of $S_j$
depends on the digits selected so far. If $S_j$ is larger, the overlap between adjacent selection
regions widens, which can compensate for the large $B^{-(j+1)}$ term even in early iterations.

To illustrate this, consider the special case where the radicand is close to $1$, leading to the
selection of $s_1, \dotsc, s_j = 0$. In this path, $S_j = S_0 = 1$, and thus $\sigma_j = 16$.
...
Substituting $B = 4$, $\alpha = 4$, $\beta = 4$, and $\sigma_j = 16$:
...
Testing this inequality for the earliest iterations:
\begin{itemize}
...
Because the condition holds for $j \geq 1$, the standard precision ($\alpha = 4$, $\beta = 4$) is
sufficient for this specific path starting from the first standard iteration.

For this path ($s_1, \dotsc, s_j = 0 \implies S_j = 1$), the residual remains negative, meaning
$\tau_j < 0$. The truncated estimates become:
...
The Radix-2 and Radix-4 case studies illustrated a complete design workflow, from interval
derivation through parameter selection to final selection logic.
