...
//*** IS THIS OPTIMAL?:
  val sSatExpQuot_S_div =
    (Mux(((7 << 9).S <= sExpQuot_S_div), 6.U, sExpQuot_S_div(12, 9)) ##
      sExpQuot_S_div(8, 0)).asSInt
  ...
  when(cyc_A1 || (cycleNum_B =/= 0.U)) {
    cycleNum_B :=
      Mux(cyc_A1, Mux(sqrtOp_PA, 10.U, 6.U), cycleNum_B - 1.U)
  }
...
//*** ADJUST CONSTANTS SO 'Fill'S AREN'T NEEDED:
    zComplK1_A7_sqrt ## Fill(10, cyc_A7_sqrt) |
      ...
      Mux((cyc_A4_sqrt && hiSqrR0_A_sqrt(9)) || cyc_A3_div, sigB_PA(46, 26) + (1 << 10).U, 0.U) |
  ...
  val mulAdd9Out_A =
    Cat(Mux(loMulAdd9Out_A(18), mulAdd9C_A(24, 18) + 1.U, mulAdd9C_A(24, 18)), loMulAdd9Out_A(17, 0))
...
//*** ONLY 30 BITS NEEDED IN CYCLE C6:
  ...
  io.mulAddC_2 :=
    Mux(cyc_B1, sigX1_B << 47, 0.U) |
      ...
      Mux(
        cyc_E3_sqrt,
        (Mux(sExp_PC(0), sigB_PC(0) << 1, (sigB_PC(1) ^ sigB_PC(0)) ## sigB_PC(0)) ^ ((~sigT_E(0)) << 1)) << 54,
        0.U
      )
  ...
  zComplSigT_C1 :=
    Mux((cyc_C1_div && !E_C1_div) || cyc_C1_sqrt, ~io.mulAddResult_3(104, 51), 0.U) |
