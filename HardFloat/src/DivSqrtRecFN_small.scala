...
class DivSqrtRawFN_small(expWidth: Int, sigWidth: Int, options: Int) extends Module {
...
//*** IS THIS OPTIMAL?:
  val sSatExpQuot_S_div =
    Cat(
      Mux(((BigInt(7) << (expWidth - 2)).S <= sExpQuot_S_div), 6.U, sExpQuot_S_div(expWidth + 1, expWidth - 2)),
  ...
  when(!idle || entering) {
    def computeCycleNum(f: UInt => UInt): UInt = {
      Mux(entering & !normalCase_S, f(1.U), 0.U) |
        Mux(
          entering_normalCase,
          Mux(io.sqrtOp, Mux(rawA_S.sExp(0), f(sigWidth.U), f((sigWidth + 1).U)), f((sigWidth + 2).U)),
          0.U
        ) |
  ...
  when(entering) {
    ...
    sign_Z := sign_S
    sExp_Z :=
      Mux(io.sqrtOp, (rawA_S.sExp >> 1) +& (BigInt(1) << (expWidth - 1)).S, sSatExpQuot_S_div)
  ...
  val rem =
    Mux(inReady && !oddSqrt_S, rawA_S.sig << 1, 0.U) |
      Mux(
        inReady && oddSqrt_S,
        Cat(rawA_S.sig(sigWidth - 1, sigWidth - 2) - 1.U, rawA_S.sig(sigWidth - 3, 0) << 3),
        0.U
      ) |
  ...
  val trialRem2 =
    Mux(newBit, (trialRem << 1) - trialTerm2_newBit1.zext, (rem_Z << 2)(sigWidth + 2, 0).zext - trialTerm2_newBit0.zext)
...
class DivSqrtRecFNToRaw_small(expWidth: Int, sigWidth: Int, options: Int) extends Module {
...
class DivSqrtRecFN_small(expWidth: Int, sigWidth: Int, options: Int) extends Module {
