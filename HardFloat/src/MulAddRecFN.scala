...
class MulAddStage1Input(expWidth: Int, sigWidth: Int) extends Bundle {
  ...
  val metadata = new MulAddMetadata(expWidth, sigWidth)
}
...
class MulAddStage2Output(expWidth: Int, sigWidth: Int) extends Bundle {
  val outSig = UInt((2 * sigWidth + 1).W)
  val metadata = new MulAddMetadata(expWidth, sigWidth)
}
...
class PreMulAddStage(expWidth: Int, sigWidth: Int) extends RawModule {
  ...
  io.out.metadata.invalidExc := IsSigNaNRawFloat(rawA) || IsSigNaNRawFloat(rawB) || IsSigNaNRawFloat(rawC) ||
    (rawA.isInf && rawB.isZero) ||
    (rawA.isZero && rawB.isInf) ||
    (!(rawA.isNaN || rawB.isNaN) && notNaN_isInfProd && rawC.isInf && doSubMags)
  io.out.metadata.isNaN := rawA.isNaN || rawB.isNaN || rawC.isNaN
  io.out.metadata.notNaN_isInfProd := notNaN_isInfProd
  io.out.metadata.notNaN_addZeros := notNaN_addZeros
  io.out.metadata.signProd := signProd
  io.out.metadata.isInfC := rawC.isInf
  io.out.metadata.sExpSum := Mux(cIsDominant, rawC.sExp, (sExpAlignedProd(expWidth + 1, 0) - sigWidth.U).asSInt)
  io.out.metadata.doSubMags := doSubMags
  io.out.metadata.cIsDominant := cIsDominant
  io.out.metadata.cDom_cAlignDist := cAlignDist(log2Ceil(sigWidth + 1) - 1, 0)
  io.out.metadata.highAlignedSigC := alignedSigC(sigSumWidth - 1, 2 * sigWidth + 1)
  io.out.metadata.bit0AlignedSigC := alignedSigC(0)
  io.out.metadata.roundingMode := io.in.roundingMode
}
...
class MulAddStage2(expWidth: Int, sigWidth: Int, columnSizes: Seq[Int]) extends RawModule {
  ...
  val io = IO(new Bundle {
    ...
    val out = Output(new MulAddStage2Output(expWidth, sigWidth))
  })
...
class PostMulAddStage(expWidth: Int, sigWidth: Int) extends RawModule {
  override def desiredName: String = s"PostMulAddStage_ew${expWidth}_sw${sigWidth}"

  val io = IO(new Bundle {
    val in = Input(new MulAddStage2Output(expWidth, sigWidth))
    val detectTininess = Input(Bool())
    val out = Output(new PostMulAddStageOutput(expWidth, sigWidth))
  })

  val sigSumWidth = 3 * sigWidth + 3

  val roundingMode_min = io.in.metadata.roundingMode === round_min

  val opSignC = io.in.metadata.signProd ^ io.in.metadata.doSubMags
  val sigSum = Cat(
    Mux(
      io.in.outSig(2 * sigWidth),
      io.in.metadata.highAlignedSigC + 1.U,
      io.in.metadata.highAlignedSigC
    ),
    io.in.outSig(2 * sigWidth - 1, 0),
    io.in.metadata.bit0AlignedSigC
  )

  val cDom_sign = opSignC
  val cDom_sExp = (io.in.metadata.sExpSum(expWidth, 0) - io.in.metadata.doSubMags).pad(expWidth + 2).asSInt
  val cDom_absSigSum = Mux(
    io.in.metadata.doSubMags,
    ~sigSum(sigSumWidth - 1, sigWidth + 1),
    Cat(false.B, io.in.metadata.highAlignedSigC(sigWidth + 1, sigWidth), sigSum(sigSumWidth - 3, sigWidth + 2))
  )
  ...
  val cDom_mainSig = (cDom_absSigSum << io.in.metadata.cDom_cAlignDist)(2 * sigWidth + 1, sigWidth - 3)
  val cDom_reduced4SigExtra = (
    OrReduceBy4(Cat(cDom_absSigSum(sigWidth - 4, 0), 0.U((~sigWidth & 3).W))) &
      LowMask(io.in.metadata.cDom_cAlignDist(log2Ceil(sigWidth + 1) - 1, 2), 0, sigWidth >> 2)
  ).orR
  val cDom_sig = Cat(
  ...
  val notCDom_absSigSum = Mux(
    ...
    sigSum(2 * sigWidth + 2, 0) + io.in.metadata.doSubMags
  )
  ...
  val notCDom_sExp = io.in.metadata.sExpSum - notCDom_nearNormDist.pad(expWidth + 2).asSInt
  ...
  val notCDom_sign = Mux(notCDom_completeCancellation, roundingMode_min, io.in.metadata.signProd ^ notCDom_signSigSum)

  val notNaN_isInfOut = io.in.metadata.notNaN_isInfProd || io.in.metadata.isInfC

  val roundRawFNToRecFN = Module(new RoundRawFNToRecFN(expWidth, sigWidth, 0))
  roundRawFNToRecFN.io.invalidExc := io.in.metadata.invalidExc
  roundRawFNToRecFN.io.infiniteExc := false.B
  roundRawFNToRecFN.io.in.isNaN := io.in.metadata.isNaN
  roundRawFNToRecFN.io.in.isInf := notNaN_isInfOut
  roundRawFNToRecFN.io.in.isZero := io.in.metadata.notNaN_addZeros ||
    (!io.in.metadata.cIsDominant && notCDom_completeCancellation)
  roundRawFNToRecFN.io.in.sign := (io.in.metadata.notNaN_isInfProd && io.in.metadata.signProd) ||
    (io.in.metadata.isInfC && opSignC) ||
    (io.in.metadata.notNaN_addZeros && !roundingMode_min && io.in.metadata.signProd && opSignC) ||
    (io.in.metadata.notNaN_addZeros && roundingMode_min && (io.in.metadata.signProd || opSignC)) ||
    (!notNaN_isInfOut && !io.in.metadata.notNaN_addZeros && Mux(io.in.metadata.cIsDominant, cDom_sign, notCDom_sign))
  roundRawFNToRecFN.io.in.sExp := Mux(io.in.metadata.cIsDominant, cDom_sExp, notCDom_sExp)
  roundRawFNToRecFN.io.in.sig := Mux(io.in.metadata.cIsDominant, cDom_sig, notCDom_sig)
  roundRawFNToRecFN.io.roundingMode := io.in.metadata.roundingMode
  roundRawFNToRecFN.io.detectTininess := io.detectTininess
  io.out.out := roundRawFNToRecFN.io.out
  io.out.exceptionFlags := roundRawFNToRecFN.io.exceptionFlags
}
...
class MulAddRecFN(expWidth: Int, sigWidth: Int, initHeight: Int) extends Module {
  ...
  if (columnSizes.max > 2) {
    val buffer1 = Module(new SkidBuffer(new MulAddStage1ToStage2(expWidth, sigWidth)))
    val stage2 = Module(new MulAddStage2(expWidth, sigWidth, columnSizes))
    val postStage = Module(new PostMulAddStage(expWidth, sigWidth))
    ...
    postStage.io.in := stage2.io.out
    postStage.io.detectTininess := io.detectTininess
