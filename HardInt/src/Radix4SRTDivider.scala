...
class DividerStage1InputWithMetadata(dataWidth: Int, tagWidth: Int) extends Bundle {
  val data = new DividerStage1Input(dataWidth)
  val metadata = new DividerMetadata(tagWidth)
}

class DividerStage1ToStage2WithMetadata(dataWidth: Int, clzWidth: Int, tagWidth: Int) extends Bundle {
  val data = new DividerStage1ToStage2(dataWidth, clzWidth)
  val metadata = new DividerMetadata(tagWidth)
}

class DividerStage2ToStage3WithMetadata(dataWidth: Int, remainderShamtWidth: Int, tagWidth: Int) extends Bundle {
  val data = new DividerStage2ToStage3(dataWidth, remainderShamtWidth)
  val metadata = new DividerMetadata(tagWidth)
}

class DividerStage3ToStage4WithMetadata(dataWidth: Int, remainderShamtWidth: Int, tagWidth: Int) extends Bundle {
  val data = new DividerStage3ToStage4(dataWidth, remainderShamtWidth)
  val metadata = new DividerMetadata(tagWidth)
}

class DividerStage4OutputWithMetadata(dataWidth: Int, tagWidth: Int) extends Bundle {
...
class PreDividerStage(dataWidth: Int, tagWidth: Int) extends RawModule with ScalarOpConstants {
  val io = IO(new Bundle {
    ...
    val out = Output(new DividerStage1InputWithMetadata(dataWidth, tagWidth))
  })
...
class DividerStage1(dataWidth: Int, clzWidth: Int, useMetadata: Boolean) extends RawModule {
  val io = IO(new Bundle {
    val in = Input(new DividerStage1Input(dataWidth))
    val isHalfWidth = Input(Bool())
    val out = Output(new DividerStage1ToStage2(dataWidth, clzWidth))
  })

  val halfWidth = dataWidth >> 1

  val dividendSign = io.in.isSigned && Mux(
    useMetadata.B && io.isHalfWidth,
    io.in.in1(halfWidth - 1),
    io.in.in1(dataWidth - 1)
  )
  val divisorSign = io.in.isSigned && Mux(
    useMetadata.B && io.isHalfWidth,
  ...
  val dividendClz = CountLeadingZeros(dividend ^ Replicate(dataWidth, dividendSign))
  val divisorClz = CountLeadingZeros(divisor ^ Replicate(dataWidth, divisorSign))

  io.out.dividend := dividend
  io.out.divisor := divisor
...
class PostDividerStage(dataWidth: Int, tagWidth: Int) extends RawModule {
  val io = IO(new Bundle {
    val in = Input(new DividerStage4OutputWithMetadata(dataWidth, tagWidth))
...
class Radix4SRTDivider(dataWidth: Int = 64, useMetadata: Boolean = true, numXPRs: Int = 32) extends Module {
  ...
  val io = IO(new Bundle {
    val req = Flipped(
      Decoupled(
        if (useMetadata) {
          new PreDividerStageInput(dataWidth, tagWidth)
        } else {
          new DividerStage1Input(dataWidth)
        }
      )
    )
    val resp = Decoupled(
      if (useMetadata) {
        new PostDividerStageOutput(dataWidth, tagWidth)
      } else {
        new DividerStage4Output(dataWidth)
      }
    )
  })
  ...
  if (useMetadata) {
    ...
    val stage1 = Module(new DividerStage1(dataWidth, clzWidth, useMetadata))
    val buffer1 = Module(new SkidBuffer(new DividerStage1ToStage2WithMetadata(dataWidth, clzWidth, tagWidth)))
    val stage2 = Module(new DividerStage2(dataWidth, clzWidth, clzDiffWidth, remainderShamtWidth, iterationWidth))
    val buffer2 = Module(
      new IterativeSkidBuffer(
        new DividerStage2ToStage3WithMetadata(dataWidth, remainderShamtWidth, tagWidth),
        iterationWidth
      )
    )
    val stage3 = Module(new DividerStage3(dataWidth, remainderShamtWidth))
    val buffer3 = Module(
      new SkidBuffer(new DividerStage3ToStage4WithMetadata(dataWidth, remainderShamtWidth, tagWidth))
    )
    ...
    stage1.io.isHalfWidth := preStage.io.out.metadata.isHalfWidth
    ...
    postStage.io.in.data := stage4.io.out
    postStage.io.in.metadata := buffer3.io.deq.bits.metadata
    io.resp.bits := postStage.io.out
    io.resp.valid := buffer3.io.deq.valid
    buffer3.io.deq.ready := io.resp.ready
  } else {
    val stage1 = Module(new DividerStage1(dataWidth, clzWidth, useMetadata))
    val buffer1 = Module(new SkidBuffer(new DividerStage1ToStage2(dataWidth, clzWidth)))
    val stage2 = Module(new DividerStage2(dataWidth, clzWidth, clzDiffWidth, remainderShamtWidth, iterationWidth))
    val buffer2 = Module(
      new IterativeSkidBuffer(new DividerStage2ToStage3(dataWidth, remainderShamtWidth), iterationWidth)
    )
    val stage3 = Module(new DividerStage3(dataWidth, remainderShamtWidth))
    val buffer3 = Module(new SkidBuffer(new DividerStage3ToStage4(dataWidth, remainderShamtWidth)))
    val stage4 = Module(new DividerStage4(dataWidth, remainderShamtWidth))

    stage1.io.in := io.req.bits
    stage1.io.isHalfWidth := DontCare
    ...
    buffer2.io.enq.valid := buffer1.io.deq.valid
    buffer1.io.deq.ready := buffer2.io.enq.ready
    buffer2.io.feedbackData := stage3.io.feedbackData
    buffer2.io.totalIterationsMinus2 := stage2.io.totalIterationsMinus2

    stage3.io.in := buffer2.io.deq.bits
    buffer3.io.enq.bits := stage3.io.out
    buffer3.io.enq.valid := buffer2.io.deq.valid
    buffer2.io.deq.ready := buffer3.io.enq.ready

    stage4.io.in := buffer3.io.deq.bits
    io.resp.bits := stage4.io.out
    io.resp.valid := buffer3.io.deq.valid
    buffer3.io.deq.ready := io.resp.ready
  }
}
