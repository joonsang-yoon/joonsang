...
class IterativeSkidBuffer[T <: Data](gen: T, iterationWidth: Int) extends Module {
  ...
  when(outputStageCanUpdate) {
    primaryDataReg := Mux(inputReadyReg, Mux(io.enq.valid, io.enq.bits, primaryDataReg), skidDataReg)
    primaryIterReg := Mux(inputReadyReg, Mux(io.enq.valid, io.totalIterationsMinus2, primaryIterReg), skidIterReg)
  }
  when(inputReadyReg) {
    skidDataReg := io.enq.bits
