...
/**
  * A bundle representing a request to the crossbar switch.
  * Contains the data payload and the destination output index.
  *
  * @tparam T The type of the data payload.
  * @param gen The model data used to generate the payload type.
  * @param nOut The total number of output ports (used to determine the width of the destination field).
  */
...
/**
  * The Interface Object (IO) for the [[XbarSwitch]].
  *
  * @tparam T The type of the data payload.
  * @param gen The model data used to generate the payload type.
...
/**
  ...
  *
  * This module routes packets from `nIn` input ports to `nOut` output ports based on a
  * destination field provided in the input bundle. It handles flow control (Ready/Valid)
  * in both directions.
  *
  * Contention for a specific output port is resolved using arbiters instantiated via the
  * `mkArbiter` factory function.
  *
  * @tparam T The type of the data payload.
  * @param gen The model data used to generate the payload type.
  * @param nIn The number of input ports. Must be > 0.
  * @param nOut The number of output ports. Must be > 0.
  * @param mkArbiter A factory function that takes an output index (Int) and returns the IO of an Arbiter.
  *                  This allows different arbitration policies (e.g., RoundRobin, Priority) to be used
  *                  or even different policies per output.
  *                  Example usage: `_ => Module(new RRArbiter(gen, nIn)).io`
  */
class XbarSwitch[T <: Data](
  ...
  // Instantiate arbiters for each output port using the provided factory function.
  private val outputArbiters: IndexedSeq[ArbiterIO[T]] = IndexedSeq.tabulate(nOut) { outputIdx =>
    mkArbiter(outputIdx)
  }

  // ---------------------------------------------------------------------------
  // 1. Forward Path (Input -> Arbiter -> Output)
  // ---------------------------------------------------------------------------
  for (outputIdx <- 0 until nOut) {
    ...
    for (inputIdx <- 0 until nIn) {
      ...
      // Assert valid to the specific arbiter only if the input is valid AND
      // the destination matches this output index.
      arbiter.in(inputIdx).valid :=
        io.in(inputIdx).valid && (io.in(inputIdx).bits.dest === outputIdx.U(log2Ceil(nOut).W))
    }

    // Connect arbiter output to the module output
    io.out(outputIdx) <> arbiter.out
    // Expose the arbiter's choice
    io.chosen(outputIdx) := arbiter.chosen
  }

  // ---------------------------------------------------------------------------
  // 2. Backward Path (Arbiter Ready -> Input Ready)
  // ---------------------------------------------------------------------------
  for (inputIdx <- 0 until nIn) {
    // Collect ready signals from all output arbiters corresponding to this input index
    val readySignals = VecInit(outputArbiters.map(_.in(inputIdx).ready))

    // The input is ready if the specific arbiter requested by 'dest' is ready.
    io.in(inputIdx).ready := readySignals(io.in(inputIdx).bits.dest)

    // Runtime assertion to ensure the destination index is within bounds
